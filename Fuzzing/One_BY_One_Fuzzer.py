...
if __name__ == '__main__':
    ...
    for k, v in hashes.items():
        original_body = hashes[k].get('body', {})
        if not original_body: continue

        print(f"{Fore.CYAN}=== Scanning API: {hashes[k]['url']} ==={Style.RESET_ALL}")

        # 針對每個參數，單獨進行測試
        for param_name in original_body:
            
            # 只測試標記為 Fuzzable 的參數
            if original_body[param_name] == "Fuzzable":
                
                print(f"Testing Parameter: {Fore.MAGENTA}{param_name}{Style.RESET_ALL}")

                # === 這裡開始是針對單一參數的 Fuzzing 邏輯 ===
                retry_count = 0
                max_retries = 3
                request_success = False

                while not request_success and retry_count < max_retries:
                    
                    # 1. [關鍵] 每次都從原始資料複製一份乾淨的 Body
                    # 這樣確保這次請求只有 param_name 是髒的，其他都是乾淨的
                    current_test_body = original_body.copy()
                    
                    # 2. 生成 Payload
                    # 建議這裡確保 payload 包含一個隨機 canary ID 以便 analyze_reflected_xss 識別
                    payload = generatePayload() 
                    
                    # 3. 只汙染當前目標參數
                    current_test_body[param_name] = payload

                    # 4. 發送請求
                    # 注意：如果 analyze 需要特定的 canary，這裡最好記錄下來
                    # 假設 analyze_reflected_xss 會去 payload 裡面找 ID
                    response = requestHandler.SendApiRequest(
                        hashes[k]['method'].upper(), 
                        hashes[k]['url'], 
                        hashes[k]['headers'], 
                        current_test_body
                    )
                    ...
                    # 5. 分析結果
                    # 這裡假設你的 analyze 函數已經能自動從 HTML 中抓取特徵
                    # 如果你需要指定 Canary ID，請從 payload 生成邏輯中取得並傳入
                    ...
                    if analyze["is_reflected"]:
                        if analyze["is_effective"]:
                            ...
                            print(f"Target URL: {hashes[k]['url']}")
                            print(f"Vulnerable Param: {Fore.RED}{param_name}{Style.RESET_ALL}") # 精準定位
                            print(f"Payload Used: {payload}")
                            print(f"Full Tag: {analyze.get('fullTag', 'N/A')}")
                            print(f"Details: {analyze['details']}")
                            
                            # 找到漏洞後，可以選擇 break 跳出這個參數的測試，或是繼續測不同 payload
                            # request_success = True # 如果想找到一個就停，取消這行註解
                        else:
                            print(f"{Fore.YELLOW}[Reflected but Sanitized] Param: {param_name}{Style.RESET_ALL}")
                    
                    # 處理重試邏輯 (避免 Server Error 無限卡住)
                    if response.status_code >= 200 and response.status_code < 500:
                        request_success = True # 請求成功處理 (包含 403/404)，換下一個參數
                    else:
                        print(f"{Fore.RED}Server Error ({response.status_code}). Retrying {param_name}...{Style.RESET_ALL}")
                        retry_count += 1